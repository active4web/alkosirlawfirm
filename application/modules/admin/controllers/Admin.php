...
public function update_visiting(){
$date= $this->db->get_where('visiting')->result();
foreach($date as $date){
    $id_data=$date->id;
     $day_t=$date->day_t;
      $month_d=$date->month_d;
       $year_d=$date->year_d;
       $final_date=$year_d."-".$month_d."-".$day_t;
       $data_up['date']=$final_date;
       $this->db->update("visiting",$data_up,array("id"=>$id_data));
...
$week_d=date_format($date,"d");
 ...
 $this->data = array( 
 'total_visitor_day'=> $this->db->get_where('visiting',array('day_t'=>$day_d,'month_d'=>$month_d,'year_d'=>$year_d))->result(),
  'total_visitor_month'=> $this->db->get_where('visiting',array('month_d'=>$month_d,'year_d'=>$year_d))->result(),
...
$work_hrs_en=$this->input->post('work_hrs_en');
$work_hrs_ar=$this->input->post('work_hrs_ar');
$data = array('work_hrs_ar'=>$work_hrs_ar,'work_hrs_en'=>$work_hrs_en,'about_footer_en'=>$about_footer_en,'about_footer_ar'=>$about_footer_ar,'message_email'=>$message_email,'address_eng'=>$address_eng,'address_ar'=>$address_ar,'name_site'=>$site_name,'name_site_ar'=>$site_name_ar,'facebook'=>$facebook,
'twitter'=>$twitter,'instagram'=>$instagram,'header_phone'=>$header_phone,'keywords'=>$key_words,'description'=>$meta_desc,'header_email'=>$header_email,'footer_phone'=>$footer_phone,'footer_email'=>$footer_email);
$this->db->update('site_info',$data,array('id'=>1));

if($_FILES['file']['name']!=""){
$file=$_FILES['file']['name'];
$file_name="file";
$config=get_img_config('site_info','uploads/site_setting/',$file,$file_name,'logo','gif|jpg|png|jpeg',600000,600000,600000,array('id'=>1),"250","75");
if($config!=0){
  $logo_site = $this->data->get_table_row('site_info',array(),'logo');
$this->session->set_userdata(array('logo_site' => $logo_site));
}
