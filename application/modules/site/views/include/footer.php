						...
						<h4><?= lang("services");?></h4>
							<div class="qanon">
								<div class="col-md-6 col-sm-6 col-xs-6 colm-2">
									<?
								$start_service=	$this->db->limit(5)->order_by('id','asc')->get_where("our_services",array("view"=>'1'))->result();
								if(count($start_service)>0){
								foreach($start_service as $start_service){
									$idservice=$start_service->id;
									?>
									<li><a href="<?=DIR?>site/services/details_services/<?= $start_service->id?>"><i class="fas fa-external-link-alt"></i>
								<?php
								if($lang == 'arabic'){
									if(strlen($start_service->title_ar)>70){	echo mb_substr($start_service->title_ar,0,70)."...";}
									else {echo mb_substr($start_service->title_ar,0,70);}
								}
								else {
								if(strlen($start_service->title_eng)>70){	echo mb_substr($start_service->title_eng,0,70)."...";}
									else {echo mb_substr($start_service->title_eng,0,70);}    
								}
								?>
								...
								<div class="col-md-6 col-sm-6 col-xs-6 colm-2">
									<?
								$last_service=	$this->db->limit(5)->order_by('id','asc')->get_where("our_services",array("view"=>'1','id>'=>$idservice))->result();
								...
								<?php
								if($lang == 'arabic'){
									if(strlen($last_service->title_ar)>70){	echo mb_substr($last_service->title_ar,0,70)."...";}
									else {echo mb_substr($last_service->title_ar,0,70);}
								}
								else {
								if(strlen($last_service->title_eng)>70){	echo mb_substr($last_service->title_eng,0,70)."...";}
									else {echo mb_substr($last_service->title_eng,0,70);}    
								}
								?>
...
<a href="<?= $siteinfo->twitter?>" target="_blank">
...
<?php } ?>
	...
	<?php if(isset($_SESSION['msg']) && $_SESSION['msg']!=''&& $this->uri->segment(3)!="regulations"){?>
<script>
$(document).ready(function(e) {
	toastr.info("<?=$_SESSION['msg'];?>",  {timeOut: 5000})
});
</script>
<?php } else if(isset($_SESSION['msg']) && $_SESSION['msg']!=''&& $this->uri->segment(3)=="regulations"){?>
...
<script>
$( document ).ready(function(e) {
$(".maindrop").mouseover(function(e) {
$(".maindrop").addClass("open");
});   

$(".maindrop").mouseout(function(e) {
