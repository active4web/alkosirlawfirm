...
<?php
		...
		$customer_id = get_table_filed('visiting',array('ip'=>$ip,'day_t'=>$day_d,'month_d'=>$month_d,'year_d'=>$year_d),'id');
		if($customer_id!=""){
		$visit_num =get_table_filed('visiting',array('ip'=>$ip,'day_t'=>$day_d,'month_d'=>$month_d,'year_d'=>$year_d),'visit_num');
		$data['visit_num']=(int)$visit_num+1;
...
<a href="<?= $site_info->twitter?>" target="_blank">
    ...
    <?php } ?>
					<div class="dropdown langmenu">
						...
						<div class="download">
<li><a href="<?=DIR?>site/pages/profiles" ><i class="fas fa-file-download fa-lg"></i><?= lang('download_file'); ?></li></a>
						</div>
					</div>
...
<li class="maindrop dropdown <?php if( $curt == 'about'||$curt == 'our_clients'){ echo 'active'; } ?>">
